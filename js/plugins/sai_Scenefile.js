...
　  info.level = $gameParty.exists() ? $gameParty.members()[0].level : null;
	  info.gold = $gameVariables.value(17);
...
//----------------------------------------------------------------------------------
    ...
    Scene_File.prototype.create = function() {
		...
		//ステータスウインドウの配置
        this._statusWindow = new Window_SavefileStatus(400, 110, 300, 330);
		...
		//ヘルプウインドウの配置
		this._helpWindow.x = 392;
		this._helpWindow.y = 75;
...
Window_SavefileList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, 100, y, 300, 380);
    this.activate();
    this._mode = null;
};
...
//現状[ファイル名+ID、プレイ時間、地名、持ち金]、名称変更は''の中の文字で
    Window_SavefileStatus.prototype.drawFileId = function(id, x, y) {
		...
		this.drawText('プレイ時間：' + ' ', 0, 200, 80);
	//	this.drawText('場所：' + ' ', 0, 170, 80);
					this.drawIcon(92, -6, 170);
    };

    Window_SavefileStatus.prototype.drawContents = function(info, x, valid) {
        this.drawText(info.playtime, 80, 200, 200);
//				if (info.gold) {
//				  this.drawText(info.gold, 80, 170, 200);
//				}
	//	if (info.location) {
	//	this.drawText(info.location, 80, 170, 200);
	//	}
	//	if (info.level) {
	//		this.drawText(TextManager.levelA + " " + info.level, 0, 245, 60);
	//	}
	//	if (typeof info.gold === 'number') {　//値が数字の時に通る
	//		this.drawText(info.gold + " " + TextManager.currencyUnit, 80, 140, 200);
	//	}


		if (valid) {
            this.drawPartyCharacters(info, 70, 280);
        }
