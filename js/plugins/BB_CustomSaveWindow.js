...
//add--------------------------------------------------------------------------
    ...
    this.contents.fillRect(rect.x - 5, rect.y + 513, rect.width + 10, 2, this.textColor(BBSWInFC));
    ...
    this.contents.fillRect(rect.x - 3, rect.y + 4, rect.width + 6, 38, this.textColor(BBSWInFC));
    this.contents.paintOpacity = BBSWOutFO;
    if (BBSWMaxItem >= 0) {
    }
    if (BBSWMaxItem >= 1) {
        this.contents.fillRect(rect.x + 200, rect.y + 215, 220, 38, this.textColor(BBSWOutFC));

        this.contents.fillRect(rect.x + 420, rect.y + 215, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 420, rect.y + 251, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 620, rect.y + 215, 2, 38, this.textColor(BBSWOutFC));

    } if (BBSWMaxItem >= 2) {
        this.contents.fillRect(rect.x + 170, rect.y + 170, 220, 38, this.textColor(BBSWOutFC));

        this.contents.fillRect(rect.x + 390, rect.y + 170, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 390, rect.y + 206, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 590, rect.y + 170, 2, 38, this.textColor(BBSWOutFC));

    } if (BBSWMaxItem >= 3) {
        this.contents.fillRect(rect.x + 140, rect.y + 125, 220, 38, this.textColor(BBSWOutFC));

        this.contents.fillRect(rect.x + 360, rect.y + 125, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 360, rect.y + 161, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 560, rect.y + 125, 2, 38, this.textColor(BBSWOutFC));
    } if (BBSWMaxItem >= 4) {
        this.contents.fillRect(rect.x + 110, rect.y + 80, 220, 38, this.textColor(BBSWOutFC));

        this.contents.fillRect(rect.x + 330, rect.y + 80, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 330, rect.y + 116, 200, 2, this.textColor(BBSWOutFC));
        this.contents.fillRect(rect.x + 530, rect.y + 80, 2, 38, this.textColor(BBSWOutFC));
    }
...
//項目0------------------------------------------------------------------------
        if (BBSWItem0 == 1) {
            this.drawText(info.title, rect.x + 42, rect.y, BBSWItem0L);
        } else if (BBSWItem0 == 2) {
            this.drawText(info.playtime, rect.x + 42, rect.y, BBSWItem0L);
        } else if (BBSWItem0 == 3) {
            this.drawText(info.mapname, rect.x + 42, rect.y, BBSWItem0L);
        } else if (BBSWItem0 == 4) {
            this.drawText(info.gold + TextManager.currencyUnit, rect.x + 42, rect.y, BBSWItem0L);
        } else if (BBSWItem0 == 5) {
            this.drawText(info.savecount, rect.x + 42, rect.y, 312);
        } else if (BBSWItem0 == 6) {
            this.drawText('プレイヤー名：' + info.value1 + BBSWV1u, rect.x + 10, rect.y + 2, 500);
        } else if (BBSWItem0 == 7) {
            this.drawText('プレイヤー名：' + nfo.value2 + BBSWV2u, rect.x + 10, rect.y + 2, 500);
        } else if (BBSWItem0 == 8) {
            this.drawText('プレイヤー名：' + info.value3 + BBSWV3u, rect.x + 10, rect.y + 2, 500);
        } else if (BBSWItem0 == 9) {
            this.drawText('プレイヤー名：' + info.value4 + BBSWV4u, rect.x + 10, rect.y + 2, 500);
            this.contents.fontSize = 18;
            this.drawText('現在のスキン', 336, 370, 300);
            this.contents.fontSize = 30;
        } else {
        }
//-----------------------------------------------------------------------------
        if (valid) {
            this.drawPartyCharacters(info, 392, 360);//-6 -12 -20
//レベル表示-------------------------------------------------------------------
        if (BBSWLOO == 1) {
...
//-----------------------------------------------------------------------------
    ...
    if (y2 >= lineHeight) {
//項目1 右下-------------------------------------------------------------------
        this.drawText(BBSWItem1T, rect.x + 210, y2 - 265, 200);
        if (BBSWItem1 == 1) {
            this.drawText(info.title, rect.x + 495 + BBSWTF1, y2 - 2, 266 - BBSWTF1, 'right');//rect.x - 7, y2 - 2, rect.width
        } else if (BBSWItem1 == 2) {
            this.drawText(info.playtime, rect.x + 430, y2 - 265, 300, 'left');
        } else if (BBSWItem1 == 3) {
...
//-----------------------------------------------------------------------------
//項目2 右上-------------------------------------------------------------------
        this.drawText(BBSWItem2T, rect.x + 180, y2 - 310, 200);
        if (BBSWItem2 == 1) {
            ...
            this.drawText(info.value1 + BBSWV1u, rect.x + 400, y2 - 310, 300, 'left');
        } else if (BBSWItem2 == 7) {
            this.drawText(info.value2 + BBSWV2u, rect.x + 400, y2 - 310, 300, 'left');
        } else if (BBSWItem2 == 8) {
            this.drawText(info.value3 + BBSWV3u, rect.x + 400, y2 - 310, 300, 'left');
        } else if (BBSWItem2 == 9) {
            this.drawText(info.value4 + BBSWV4u, rect.x + 400, y2 - 310, 300, 'left');
        } else {
        }
//-----------------------------------------------------------------------------
//項目3 左下-------------------------------------------------------------------
        this.drawText(BBSWItem3T, rect.x + 150, y2 - 355, 200);
        if (BBSWItem3 == 1) {
            ...
            this.drawText(info.value1 + BBSWV1u, rect.x + 370, y2 - 355, 300, 'left');
        } else if (BBSWItem3 == 7) {
            this.drawText(info.value2 + BBSWV2u, rect.x + 370, y2 - 355, 300, 'left');
        } else if (BBSWItem3 == 8) {
            this.drawText(info.value3 + BBSWV3u, rect.x + 370, y2 - 355, 300, 'left');
        } else if (BBSWItem3 == 9) {
            this.drawText(info.value4 + BBSWV4u, rect.x + 370, y2 - 355, 300, 'left');
        } else {
        }
//-----------------------------------------------------------------------------
//項目4 1-------------------------------------------------------------------
        this.drawText(BBSWItem4T, rect.x + 120, y2 - 400, 200);
        if (BBSWItem4 == 1) {
            this.drawText(info.title, rect.x + 215 + BBSWTF4, y2 - 300, 266 - BBSWTF4, 'right');//rect.x - 287, y2 - 34, rect.width
        } else if (BBSWItem4 == 2) {
            this.drawText(info.playtime, rect.x + 215 + BBSWTF4, y2 - 300, 266 - BBSWTF4, 'right');
        } else if (BBSWItem4 == 3) {
            this.drawText(info.mapname, rect.x + 215 + BBSWTF4, y2 - 300, 266 - BBSWTF4, 'right');
        } else if (BBSWItem4 == 4) {
            this.drawText(info.gold + TextManager.currencyUnit, rect.x + 300 + BBSWTF4, y2 - 334, 266 - BBSWTF4, 'right');
        } else if (BBSWItem4 == 5) {
            this.drawText(info.savecount, rect.x + 215 + BBSWTF4, y2 - 300, 266 - BBSWTF4, 'right');
        } else if (BBSWItem4 == 6) {
            this.drawText(info.value1 + BBSWV1u, rect.x + 340, y2 - 400, 300, 'left');
        } else if (BBSWItem4 == 7) {
            this.drawText(info.value2 + BBSWV2u, rect.x + 340, y2 - 400, 300, 'left');
        } else if (BBSWItem4 == 8) {
            this.drawText(info.value3 + BBSWV3u, rect.x + 340, y2 - 400, 300, 'left');
        } else if (BBSWItem4 == 9) {
            this.drawText(info.value4 + BBSWV4u, rect.x + 340, y2 - 400, 300, 'left');
        } else {
        }
//-----------------------------------------------------------------------------
//項目5 上部-------------------------------------------------------------------
